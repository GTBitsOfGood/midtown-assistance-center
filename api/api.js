...
app.get('/onlineTutors', (req, res) => {
    function addActiveSession(tutor) {
        ...
        var getSession = new Promise(function(resolve, reject) {
            data_access.tutor_sessions.getActiveSession(return_tutor._id, function(
                err,
                response
            ) {
                if (err) {
...
app.get('/confirmEmail', (req, res) => {
    ...
    data_access.users.confirmEmail({_id: tutor_id, confirm_key:confirm_key}, function(err, resultTutor) {
        if (err) {
            console.error(err);
            return res.json({
                success: false,
