...
class TutorModal extends React.Component {
    ...
    handleSubmitRequest() {
        ...
        axios.post('/api/addJoinRequest', requestBody)
            .then(function(response){
                if (response.data.success) {
                    console.log(response.data);
                    this.props.socket.emit('student-request', {'session':this.props.data.session.eventId, 'student': this.props.username, 'topic': this.state.topic, 'request': this.state.request});
                } else {
    ...
    render() {
        ...
        const approved_html = <div>
            ...
            <span onMouseOver = {() => this.changeStar(1)} onMouseOut = {this.changeStarOut} onClick={() => this.setRating(1)}><img className="star" src={this.state.first_star ? '/images/full-star.png' : '/images/empty-star.png'}/></span>
            <span onMouseOver = {() => this.changeStar(2)} onMouseOut = {this.changeStarOut}  onClick={() => this.setRating(2)}><img className="star" src={this.state.second_star ? '/images/full-star.png' : '/images/empty-star.png'}/></span>
            <span onMouseOver = {() => this.changeStar(3)} onMouseOut = {this.changeStarOut}  onClick={() => this.setRating(3)}><img className="star" src={this.state.third_star ? '/images/full-star.png' : '/images/empty-star.png'}/></span>
            <span onMouseOver = {() => this.changeStar(4)} onMouseOut = {this.changeStarOut}  onClick={() => this.setRating(4)}><img className="star" src={this.state.fourth_star ? '/images/full-star.png' : '/images/empty-star.png'}/></span>
            <span onMouseOver = {() => this.changeStar(5)} onMouseOut = {this.changeStarOut}  onClick={() => this.setRating(5)}><img className="star" src={this.state.fifth_star ? '/images/full-star.png' : '/images/empty-star.png'}/></span>
            <span><h4 className="rating-span">({this.state.rating}/5) {this.state.satisfaction}</h4></span>
            <h5>Leave some feedback (optional)</h5>
            <textarea value={this.state.comment} onChange={this.handleCommentChange} className="input-sm input feedback-text"/>
        </div>
        const pending_html = <div>
            <h5>What can {this.props.firstName} help you with?</h5>
            <select onChange={this.handleTopicChange} className="input input-sm">
                {subject_select}
                {favorite_select}
            </select>
            <h5>More Details (Optional)</h5>
            <textarea value={this.state.request} onChange={this.handleRequestChange} className="input-sm input feedback-text"/>
        </div>
        return (
            <div>
                        ...
                        <div className="modal-content review-modal">
                            ...
                            <div className="modal-body text-center">
                                <div id={"ModalBody_" + this.props.firstName}>
                                    {this.state.approval === 'approved' ? approved_html : pending_html}
                                </div>
                            </div>
                            <div className="review-modal-footer modal-footer">
                                ...
                                <button type="submit" onClick={this.state.approval === 'approved' ? this.handleSubmit : this.handleSubmitRequest} className="btn btn-default mac_button_inverse">Submit</button>
                            </div>
                        </div>
                    </div>
                </div>
